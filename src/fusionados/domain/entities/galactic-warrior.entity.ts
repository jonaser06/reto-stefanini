/**
 * interfaz de un guerrero galactico
 */
export interface GalacticWarrior {
  id: string; // id unico
  name: string; // nombre
  powerLevel: number; // nivel de poder
  origin: "pokemon" | "starwars"; // origen
...
export class GalacticWarriorEntity implements GalacticWarrior {
  ...
  /**
   * crear guerrero fusion pokemon + star wars
   */
  static fromPokemonAndStarWars(
    pokemon: any,
    starWarsCharacter: any
  ): GalacticWarriorEntity {
    const id = `${pokemon.id}-${
      starWarsCharacter.url.split("/").slice(-2, -1)[0]
    }`;

    const fusedName = `${
      pokemon.name.charAt(0).toUpperCase() + pokemon.name.slice(1)
    } ${starWarsCharacter.name.split(" ")[0]}`;
    ...
    const pokemonHeight = pokemon.height ? pokemon.height / 10 : 1.0; // decimetros a metros
    const characterHeight = parseInt(starWarsCharacter.height) / 100 || 1.7; // cm a metros
    const avgHeight = (pokemonHeight + characterHeight) / 2;

    const pokemonWeight = pokemon.weight ? pokemon.weight / 10 : 50; // hectogramos a kg
    const characterWeight = parseInt(starWarsCharacter.mass) || 75;
    const avgWeight = (pokemonWeight + characterWeight) / 2;

    const pokemonAbilities =
      pokemon.abilities?.map((ability: any) => ability.ability.name) || [];
    const characterOrigin = starWarsCharacter.homeworld
      ? ["Force-sensitive"]
      : ["Galactic-warrior"];
    ...
    const species = `${
      pokemonSpecies.charAt(0).toUpperCase() + pokemonSpecies.slice(1)
    }-Humanoid`;
  ...
  /**
   * crear guerrero solo de pokemon
   */
  ...
  /**
   * crear guerrero solo de star wars
   */
